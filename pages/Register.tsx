...
import { Hotel, User as UserIcon, Mail, Lock, Building, ArrowLeft, CheckCircle2, Info } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6">
        <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md p-10 text-center space-y-6 border border-emerald-100 animate-in zoom-in-95 duration-500">
          ...
          <h2 className="text-2xl font-black text-slate-900">Controlla la tua Email</h2>
          <p className="text-slate-500 font-medium">Abbiamo inviato un link di conferma a <span className="text-emerald-600 font-bold">{email}</span>. Clicca sul link per attivare la tua struttura.</p>
          <button 
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-6 py-12">
      <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] w-full max-w-xl p-8 md:p-12 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 border border-slate-100">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-400 font-bold text-xs mt-2 uppercase tracking-[0.2em]">Configurazione Master Owner</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="w-full px-5 py-3.5 rounded-2xl border-2 border-slate-50 focus:border-emerald-500 focus:bg-white outline-none transition-all font-semibold text-slate-800 bg-slate-50/50" placeholder="Grand Hotel" required />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome Titolare</label>
              <input type="text" value={ownerName} onChange={(e) => setOwnerName(e.target.value)} className="w-full px-5 py-3.5 rounded-2xl border-2 border-slate-50 focus:border-emerald-500 focus:bg-white outline-none transition-all font-semibold text-slate-800 bg-slate-50/50" placeholder="Nome Cognome" required />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Crea Password</label>
              <div className="relative">
          ...
          <button type="submit" disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-5 rounded-2xl transition-all shadow-xl shadow-emerald-600/20 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
            {loading ? 'Elaborazione...' : 'Crea Account & Invia Email'}
            {!loading && <CheckCircle2 size={18} />}
          </button>

          <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <Info size={16} className="text-emerald-600 shrink-0" />
            <p className="text-[9px] font-bold text-slate-400 leading-relaxed uppercase tracking-wider">Riceverai un'email di conferma. L'account non sarà attivo finché non cliccherai sul link ricevuto.</p>
          </div>
        </form>
      </div>
    </div>
