...
import { ArrowLeft, Save, Plus, Minus, Shirt, PenTool, Clock } from 'lucide-react';
import { Property, InventoryItem, Product } from '../types';

interface LaundryInventoryFormProps {
  property: Property;
...
export const LaundryInventoryForm: React.FC<LaundryInventoryFormProps> = ({
  property,
  availableProducts,
  currentStock,
  lastSubmission,
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in zoom-in-95 duration-300 pb-12">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-800">Inventario Biancheria: {property.name}</h2>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start gap-3 shadow-sm">
          ...
          <div className="text-sm">
              <span className="font-bold text-blue-800 uppercase tracking-tight">Ultimo Log Biancheria:</span>
              {lastSubmission ? (
                  ...
                  <p className="text-blue-600 mt-0.5 italic">Nessun log biancheria registrato per questa struttura.</p>
              )}
          </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-blue-600 text-xs uppercase font-bold text-white tracking-widest">
              <tr>
                <th className="px-6 py-4">Biancheria</th>
                <th className="px-6 py-4 text-center">Conteggio</th>
              </tr>
            </thead>
                  ...
                  <tr key={item.productId} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center space-x-4">
                        ...
                        <span className="w-10 text-center font-black text-xl text-blue-700">{item.quantityInStock}</span>
                        <button 
                          onClick={() => updateQuantity(index, 1)}
                          className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 hover:bg-blue-100 transition-all active:scale-90 shadow-sm"
                        >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 shadow-inner">
             ...
             <input 
                ...
                className="w-full px-4 py-4 border border-gray-300 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all bg-white font-serif italic text-2xl text-slate-700"
             />
        </div>
      </div>
      ...
      <div className="flex justify-end pt-4">
        <button
          ...
          className={`flex items-center px-10 py-4 rounded-xl text-white font-bold shadow-xl transition-all ${
            signatureName.trim() ? 'bg-blue-600 hover:bg-blue-700 hover:scale-[1.02] shadow-blue-600/30' : 'bg-slate-300 shadow-none cursor-not-allowed'
          }`}
          disabled={!signatureName.trim()}
        >
          <Save className="w-5 h-5 mr-2" />
          Salva e Invia Report
        </button>
      </div>
    </div>
  );
